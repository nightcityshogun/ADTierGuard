...
<#
.SYNOPSIS
    ADTierGuard v4.3 - Pure ADSI Forest-Wide Tier Isolation
    
.DESCRIPTION
    100% Pure ADSI/P-Invoke - NO ActiveDirectory or GroupPolicy modules required.
    
    Key Research Findings Applied:
    - msDS-UserAllowedToAuthenticateFrom: attributeSyntax 2.5.5.10 (octet string/binary)
    - Conditional ACE format: XA;OICI;CR;;;WD;(condition)
    - ED = Enterprise Domain Controllers (S-1-5-9)
    - Member_of / Member_of_any require SID() wrapper
    - Registry.pol: UTF-16LE with PReg header, bracketed entries
    - Kerberos Armoring: KDC\Parameters keys for DCs, Kerberos\Parameters for clients
...
.NOTES
    Version: 4.3.0
    Based on: Kili69/Tier0-User-Management, canix1/T0AuthSilo
...
function New-ADSIGMSA {
    ...
    $searcher = New-Object System.DirectoryServices.DirectorySearcher([ADSI]"LDAP://$Server/$container")
    $searcher.Filter = "(&(objectClass=msDS-GroupManagedServiceAccount)(sAMAccountName=$Name`$))"
    if ($searcher.FindOne()) { Write-Log "    GMSA exists: $Name" -Level Info; return }
    ...
    try {
        ...
        $dcSid = (New-Object System.Security.Principal.SecurityIdentifier([byte[]]$dcSearcher.FindOne().Properties['objectSid'][0], 0)).Value.Trim()
    ...
    } catch { Write-Log "    Failed GMSA: $Name - $_" -Level Error }
}
...
Write-Host "  5. Enable scheduled tasks GPO after testing" -ForegroundColor White
Write-Host ""
