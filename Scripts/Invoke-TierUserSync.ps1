...
.DESCRIPTION
    Uses forest topology to properly handle:
    - Domain-scoped groups (Domain Admins, Account Operators, etc.)
    - Forest-scoped groups (Enterprise Admins, Schema Admins) - only in forest root
    - Builtin groups (Administrators, Backup Operators, etc.)
#>
function Invoke-PrivilegedGroupCleanup {
    ...
    Write-SyncLog -Message "Allowed accounts for privileged groups: $($allowedAccounts.Count)" -Level Verbose
    
    # Initialize forest topology
    try {
        $null = Initialize-ForestTopology
    }
    catch {
        Write-SyncLog -Message "Failed to initialize forest topology: $($_.Exception.Message)" -Level Warning
        Write-SyncLog -Message "Falling back to adminCount-based group enumeration" -Level Warning
        
        # Fallback to old behavior
        return Invoke-PrivilegedGroupCleanupFallback -DomainDN $DomainDN -Server $Server `
            -TierAdmins $TierAdmins -ServiceAccounts $ServiceAccounts
    }
    
    # Get privileged groups for this domain (includes forest groups if this is forest root)
    $isForestRoot = Test-IsForestRoot -DomainDnsName $DomainDnsName
    
    if ($isForestRoot) {
        Write-SyncLog -Message "Domain '$DomainDnsName' is FOREST ROOT - including Enterprise/Schema Admins" -Level Information
    }
    else {
        Write-SyncLog -Message "Domain '$DomainDnsName' is child/tree domain - domain groups only" -Level Information
    }
    
    $privilegedGroups = Get-AllPrivilegedGroupsForDomain -DomainDnsName $DomainDnsName
    
    Write-SyncLog -Message "Checking $($privilegedGroups.Count) privileged groups" -Level Information
    
    foreach ($groupInfo in $privilegedGroups) {
        $stats.GroupsChecked++
        
        # Resolve group DN from SID
        $groupDN = $null
        try {
            if ($groupInfo.IsBuiltin) {
                # Builtin groups are in CN=Builtin
                $groupDN = Get-GroupDNFromSID -SID $groupInfo.SID -Server $Server -DomainDN $DomainDN
            }
            else {
                $groupDN = Get-GroupDNFromSID -SID $groupInfo.SID -Server $Server -DomainDN $DomainDN
            }
            ...
            Write-SyncLog -Message "Cannot resolve group '$($groupInfo.Name)' (SID: $($groupInfo.SID)): $($_.Exception.Message)" -Level Warning
            continue
        }
        
        if (-not $groupDN) {
            Write-SyncLog -Message "Group not found: $($groupInfo.Name) (SID: $($groupInfo.SID))" -Level Verbose
            continue
        }
        
        # Track forest vs domain groups
        if ($groupInfo.RID -eq 518 -or $groupInfo.RID -eq 519) {
            $stats.ForestGroups++
            Write-SyncLog -Message "Checking FOREST group: $($groupInfo.Name)" -Level Information
        }
        else {
            $stats.DomainGroups++
            Write-SyncLog -Message "Checking domain group: $($groupInfo.Name)" -Level Verbose
        }
        
        # Get group members
        $members = Get-AdsiGroupMember -GroupDistinguishedName $groupDN -Server $Server
        ...
        foreach ($memberDN in $members) {
            ...
            try {
                ...
                Write-SyncLog -Message "UNAUTHORIZED: $($memberObj.sAMAccountName) in $($groupInfo.Name)" -Level Warning
                
                if ($PSCmdlet.ShouldProcess($memberObj.sAMAccountName, "Remove from $($groupInfo.Name)")) {
                    try {
                        ...
                        $stats.Removed++
                        Write-SyncLog -Message "Removed $($memberObj.sAMAccountName) from $($groupInfo.Name)" `
    ...
    Write-SyncLog -Message "Privileged group cleanup: Checked $($stats.GroupsChecked) groups ($($stats.ForestGroups) forest, $($stats.DomainGroups) domain), removed $($stats.Removed), errors $($stats.Errors)" -Level Information
...
<#
.SYNOPSIS
    Resolves a group DN from its SID.
#>
function Get-GroupDNFromSID {
    [CmdletBinding()]
    [OutputType([string])]
    param(
        [Parameter(Mandatory)]
    ...
    try {
        ...
        $filter = "(&(objectClass=group)(objectSid=$sidHex))"
        ...
        # Try searching in CN=Builtin for builtin groups
        $builtinDN = "CN=Builtin,$DomainDN"
        $results = Search-AdsiDirectory -SearchBase $builtinDN -LdapFilter $filter `
            -Properties @('distinguishedName') -Server $Server -SearchScope 'OneLevel'
        ...
        Write-Verbose "Failed to resolve SID $SID : $($_.Exception.Message)"
    }
...
<#
.SYNOPSIS
    Fallback privileged group cleanup using adminCount attribute.
#>
function Invoke-PrivilegedGroupCleanupFallback {
    [CmdletBinding(SupportsShouldProcess)]
    [OutputType([hashtable])]
    param(
        [Parameter(Mandatory)]
    ...
    # Get privileged groups (adminCount=1) - fallback method
    Write-SyncLog -Message "Using fallback: enumerating groups with adminCount=1" -Level Warning
    ...
    foreach ($group in $privilegedGroups) {
        ...
        foreach ($memberDN in $members) {
            ...
            try {
                $memberObj = Get-AdsiObject -DistinguishedName $memberDN `
                    -Properties @('objectClass', 'sAMAccountName') -Server $Server
                
                if ($memberObj.objectClass -contains 'group') { continue }
                if ($memberObj.objectClass -contains 'msDS-GroupManagedServiceAccount') { continue }
                ...
                if ($PSCmdlet.ShouldProcess($memberObj.sAMAccountName, "Remove from $($group.sAMAccountName)")) {
                    Remove-AdsiGroupMember -GroupDistinguishedName $group.distinguishedName `
                        -MemberDistinguishedName $memberDN -Server $Server -Confirm:$false
                    $stats.Removed++
                }
            }
            catch {
...
function Invoke-DomainUserSync {
    ...
    if (-not $SkipPrivilegedGroupCleanup -and $TierLevel -eq 0 -and $TierConfig.EnforcePrivilegedGroupCleanup) {
        $cleanupResult = Invoke-PrivilegedGroupCleanup -DomainDN $domainDN -DomainDnsName $domainDns `
            -Server $domainDns -TierAdmins $adminUsers.ToArray() -ServiceAccounts $serviceAccounts.ToArray()
    }
...
try {
    ...
    # =========================================================================
    # VALIDATE AUTHENTICATION POLICIES EXIST (Forest-wide in Configuration NC)
    # =========================================================================
    # Authentication Policies are stored in the Configuration partition and are
    # FOREST-WIDE. They must be created ONCE from the forest root domain using
    # Initialize-TierAuthenticationPolicy before sync can apply them.
    # =========================================================================
    
    $policyName = $Script:TierConfig.KerberosAuthPolicyName
    if ($policyName) {
        Write-SyncLog -Message "Validating Authentication Policy: $policyName" -Level Information
        
        # Get forest context to show where policies are stored
        $forestContext = Get-AuthPolicyForestContext
        Write-SyncLog -Message "  Policy Storage: $($forestContext.AuthPoliciesPath)" -Level Verbose
        Write-SyncLog -Message "  Forest Root: $($forestContext.ForestRootDns)" -Level Verbose
        Write-SyncLog -Message "  Current Domain: $($forestContext.CurrentDomainDns) (IsForestRoot: $($forestContext.IsForestRoot))" -Level Verbose
        
        # Check if policy exists
        $policy = Get-TierAuthenticationPolicy -Name $policyName
        
        if (-not $policy -or $policy.Count -eq 0) {
            Write-SyncLog -Message @"
AUTHENTICATION POLICY NOT FOUND: '$policyName'

Authentication Policies are FOREST-WIDE and must be created ONCE from the forest root domain.

To create the policy, run the following from a forest root DC as Enterprise Admin:

    Initialize-TierAuthenticationPolicy -TierLevel $TierLevel -UseSilo

Or to create just the policy without a silo:

    New-TierAuthenticationPolicy -Name '$policyName' -TGTLifetimeMinutes 240 -Enforced

Policy storage location:
    $($forestContext.AuthPoliciesPath)

"@ -Level Error -EventId $eventIds.UserSyncError
            throw "Authentication Policy '$policyName' not found in forest Configuration NC."
        }
        else {
            $policyDN = $policy[0].distinguishedName
            $enforced = if ($policy[0].enforced) { "ENFORCED" } else { "AUDIT-ONLY" }
            Write-SyncLog -Message "  [OK] Policy found: $policyDN [$enforced]" -Level Information
        }
    }
    else {
        Write-SyncLog -Message "No KerberosAuthPolicyName configured for Tier $TierLevel - skipping policy application" -Level Warning
    }
    ...
    foreach ($d in $targetDomains) {
        $typeInfo = if ($d.IsForestRoot) { "[FOREST ROOT]" } elseif ($d.IsChildDomain) { "[Child]" } elseif ($d.IsTreeRoot) { "[Tree Root]" } else { "" }
        Write-SyncLog -Message "  - $($d.DnsName) $typeInfo" -Level Verbose
    }
