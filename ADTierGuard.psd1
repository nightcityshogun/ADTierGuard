...
A comprehensive solution for Active Directory Tier 0/1 isolation using 
Kerberos Authentication Policies. Supports both centralized management 
and distributed GPO-based deployment to Domain Controllers.

Key Features:
- Pure ADSI implementation (no ActiveDirectory module dependency)
- GPO-based deployment with scheduled tasks on Domain Controllers
- Group Managed Service Account (GMSA) support for secure execution
- Runspace-based parallel processing for high performance
- Automatic computer group membership for auth policy claims
- Kerberos Authentication Policy enforcement
- Protected Users group management
- Privileged group cleanup for Tier 0 accounts
- Kerberos Armoring (FAST) configuration
- SYSVOL-based script and configuration deployment
...
Deployment Models:
1. Centralized: Run from management server (Invoke-TierUserSync/Invoke-TierComputerSync)
2. Distributed: GPO pushes scheduled tasks to all DCs (Install-TierGuardForest)
