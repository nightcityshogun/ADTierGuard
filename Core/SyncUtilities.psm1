...
.EXAMPLE
    $domains = Get-SyncTargetDomains -Configuration $config -ForestScope $true
#>
function Get-SyncTargetDomains {
    ...
    param(
        ...
        [Parameter()]
        [bool]$ForestScope = $false
    )
    
    if ($ForestScope -or $Configuration.General.ForestScope) {
        # Get all forest domains
        $allDomains = Get-AdsiForestDomains
        
        # Apply whitelist filter
        if ($Configuration.Domains.TargetDomains -and $Configuration.Domains.TargetDomains.Count -gt 0) {
            $targetList = $Configuration.Domains.TargetDomains
            $allDomains = $allDomains | Where-Object {
                $_.DnsName -in $targetList -or $_.NetBIOSName -in $targetList
            }
        }
        
        # Apply blacklist filter
        if ($Configuration.Domains.ExcludedDomains -and $Configuration.Domains.ExcludedDomains.Count -gt 0) {
            $excludeList = $Configuration.Domains.ExcludedDomains
            $allDomains = $allDomains | Where-Object {
                $_.DnsName -notin $excludeList -and $_.NetBIOSName -notin $excludeList
            }
        }
        ...
        # Single domain mode - current domain only
        $rootDse = Get-AdsiRootDse
        $defaultNC = $rootDse.DefaultNamingContext
        
        # Extract domain DNS name from RootDSE
        $dnsName = $rootDse.DnsHostName
        if ($dnsName -match '^[^.]+\.(.+)$') {
            $dnsName = $Matches[1]
        }
        
        # Extract NetBIOS name from DN
        $netbiosName = ($defaultNC -split ',' | 
