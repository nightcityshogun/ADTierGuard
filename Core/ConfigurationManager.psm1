...
.DESCRIPTION
    ...
    Configuration can be stored locally or in SYSVOL for domain-wide access.
    Default SYSVOL path: \\<domain>\SYSVOL\<domain>\scripts\ADTierGuard.config
...
# Module constants
$script:ConfigFileName = 'ADTierGuard.config'
#endregion

#region SYSVOL Path Functions

<#
.SYNOPSIS
    Gets the SYSVOL path for configuration file.
.DESCRIPTION
    Returns the UNC path to the configuration file in SYSVOL.
    Pattern: \\<domain>\SYSVOL\<domain>\scripts\ADTierGuard.config
.PARAMETER DomainDnsName
    The DNS name of the domain. If not specified, uses current domain.
.OUTPUTS
    String - UNC path to configuration file in SYSVOL
#>
function Get-SysvolConfigPath {
    [CmdletBinding()]
    [OutputType([string])]
    param(
    ...
    if (-not $DomainDnsName) {
        try {
            $DomainDnsName = [System.DirectoryServices.ActiveDirectory.Domain]::GetCurrentDomain().Name
        }
        catch {
            Write-Warning "Cannot determine current domain. Specify -DomainDnsName parameter."
            return $null
        }
    }
    
    return "\\$DomainDnsName\SYSVOL\$DomainDnsName\scripts\$script:ConfigFileName"
}

<#
.SYNOPSIS
    Gets the SYSVOL scripts path for ADTierGuard.
.DESCRIPTION
    Returns the UNC path to the scripts folder in SYSVOL.
.PARAMETER DomainDnsName
    The DNS name of the domain. If not specified, uses current domain.
.OUTPUTS
    String - UNC path to scripts folder in SYSVOL
#>
function Get-SysvolScriptPath {
    [CmdletBinding()]
    [OutputType([string])]
    param(
        [Parameter(Mandatory = $false)]
    ...
    if (-not $DomainDnsName) {
        try {
            ...
            Write-Warning "Cannot determine current domain."
            return $null
        }
    }
    
    return "\\$DomainDnsName\SYSVOL\$DomainDnsName\scripts"
}
...
.DESCRIPTION
    ...
    2. Current directory
    3. SYSVOL scripts folder
    4. Module directory
.PARAMETER Path
    Optional specific path to check first.
.OUTPUTS
    String - Path to configuration file, or $null if not found
#>
function Find-TierGuardConfigFile {
    [CmdletBinding()]
    [OutputType([string])]
    param(
        [Parameter(Mandatory = $false)]
    ...
    # 2. Current directory
    $searchPaths += Join-Path $PWD $script:ConfigFileName
    
    # 3. SYSVOL
    $sysvolPath = Get-SysvolConfigPath
    if ($sysvolPath) {
        $searchPaths += $sysvolPath
    }
    
    # 4. Module directory
    $moduleDir = Split-Path -Parent $PSScriptRoot
    $searchPaths += Join-Path $moduleDir "Config\$script:ConfigFileName"
    $searchPaths += Join-Path $moduleDir "Config\Sample-TierGuard.json"
    
    foreach ($testPath in $searchPaths) {
        if ($testPath -and (Test-Path $testPath -ErrorAction SilentlyContinue)) {
            Write-Verbose "Found configuration file: $testPath"
            return $testPath
        }
    }
    
    Write-Warning "Configuration file not found in any standard location"
    return $null
}
...
function Get-ConfigurationSchema {
    ...
    return @{
        ...
        General = @{
            ForestScope              = $true           # Process all domains in forest (auto-discovered)
            ExcludeDomains           = @()             # Domains to exclude from processing
        ...
        Tier0 = @{
            ...
            EnforcePrivilegedGroupCleanup = $true       # Clean Enterprise/Schema Admins in forest root
            IncludeDomainControllers     = $true        # Include DCs in computer group
        ...
        Scheduling = @{
            ...
            EnabledTasks                = @('Tier0-ComputerSync', 'Tier0-UserSync')
        }
...
Export-ModuleMember -Function @(
    ...
    'Get-SysvolConfigPath'
    'Get-SysvolScriptPath'
    'Find-TierGuardConfigFile'
